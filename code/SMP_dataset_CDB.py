...
def validation(epoch, model, data_loader, criterion, device):
    ...
    with torch.no_grad():
        ...
        with tqdm(total=len(data_loader)) as pbar:
            ...
            for step, (images, masks, _) in enumerate(data_loader):
                ...
                loss = torch.mean(loss)    # CDB Loss 용
                total_loss += loss
                cnt += 1
        ...
        class_wise_iou = np.array(class_wise_iou)
    ...
    return avrg_loss, mIoU, class_wise_iou

def save_model(model, saved_dir, file_name='SMP_PAN_SwinL_StepLR_AUG2_CDBloss_best_model(pretrained).pt'):    ########## 저장 파일 이름 바꿔주세요 ##########
...
def train(num_epochs, model, data_loader, val_loader, criterion, optimizer, scheduler, saved_dir, val_every, device):
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % val_every == 0:
            avrg_loss, val_mIoU, class_wise_iou = validation(epoch + 1, model, val_loader, criterion, device)

            # For CDB Loss - CDB Loss 안쓸때에는 주석처리 해주세요
            criterion = CDB_loss(class_difficulty = 1 - class_wise_iou, tau = 'dynamic').cuda()
